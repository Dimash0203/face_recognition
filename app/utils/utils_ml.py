...
def read_image(file: UploadFile) -> np.ndarray:
    if file.content_type not in ALLOWED_MIME:
        raise HTTPException(415, detail=f"Неподдерживаемый тип файла: {file.content_type}")
    data = file.file.read(settings.max_image_bytes + 1)
    if len(data) == 0:
        raise HTTPException(400, detail="Файл пустой")
    if len(data) > settings.max_image_bytes:
        raise HTTPException(413, detail=f"Файл слишком большой (>{settings.max_image_bytes} байт)")
...
def read_image_b64(b64: str) -> np.ndarray:
    ...
    if m:
        ...
        if mime.lower() not in ALLOWED_MIME:
            raise HTTPException(415, detail=f"Неподдерживаемый тип в data URL: {mime}")
    ...
    try:
        ...
        raise HTTPException(400, detail="Некорректная base64-строка")

    if len(raw) == 0:
        raise HTTPException(400, detail="Пустые данные изображения (base64)")
    ...
    img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
    if img is None:
        raise HTTPException(400, detail="Не удалось декодировать изображение (base64)")
    return img
